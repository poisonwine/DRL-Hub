...
from myenvs.fetch import robot_env_m
from myenvs.fetch import robot_env
...
class CatchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        #
        # return sparse_reward, False, info
        #
        d = goal_distance(achieved_goal, goal)
        if self.reward_type == 'sparse':
            # info['is_success'] = np.where(d > self.distance_threshold, False, True)
            # info['is_success'] = self._is_success(achieved_goal, goal)
            return -(d > self.distance_threshold).astype(np.float32)
        else:
            return np.where(d > self.distance_threshold, -d, -d + 1.0)

    def _restart_target(self):
        target_x = self.np_random.uniform(low=0.64, high=0.84)
        target_y = self.np_random.uniform(low=-0.23, high=0.23)
        target_z = self.np_random.uniform(low=0.2, high=0.56)
        self.end_location = [1.8 - target_x, 0.75 + target_y, target_z]

        y = self.np_random.uniform(low=-0.5, high=0.5)
        z = self.np_random.uniform(low=-0.1, high=0.1)
        z_offset = 0.5 + z
    ...
    def _get_obs(self):
        ...
        self.goal = self.sim.data.get_site_xpos('tar')
        out_dict = {
